//JERSEY CATEGORY COMPONENT
//THIS PAGE WILL DISPLAY THE JERSEY PRODUCTS
...
import jerseyAlt from '../Assets/webp-img/jersey-alt.webp'
...
const Jersey = () => {
  ...
  const productRef = collection(db, 'products')
   useEffect(() => {
   const jerseyProductsQuery = query(productRef, where("productCategory", "==", "JERSEY"));
    
    getDocs(jerseyProductsQuery)
      .then((snapshot) => {
        let productlists = []
        snapshot.docs.forEach((doc) => {
          productlists.push({...doc.data(), id: doc.id}) 
        })
        setJerseyProduct(productlists);
      })
      .catch(err => {
  ...
  return (
    ...
    <h1 className='text-xl'>Jersey</h1>

    <div className='grid gap-4 2xl:grid-cols-3 my-2'>

   {jerseyProduct.map(product => (
  <button onClick={() => handleClick(product)}>
  <div className='border p-2 bg-gray-300 rounded-lg uppercase flex items-center '>
    
        <div>
        {/* <img className='w-36 h-36 rounded-md' src={product.productImg} alt={jerseyAlt} /> */}
        <img className='w-36 h-36 rounded-md' src={tempImg} alt={jerseyAlt} />
        </div>
