//PATCH CATEGORY COMPONENT
//THIS PAGE WILL DISPLAY THE PATCH PRODUCTS
...
import patchAlt from '../Assets/webp-img/patch-alt.webp'
...
const Patch = () => {
  ...
  const productRef = collection(db, 'products')
   useEffect(() => {
   const patchProductsQuery = query(productRef, where("productCategory", "==", "PATCH"));
    
    getDocs(patchProductsQuery)
      .then((snapshot) => {
        let productlists = []
        snapshot.docs.forEach((doc) => {
          productlists.push({...doc.data(), id: doc.id}) 
        })
        setPatchProduct(productlists);
      })
      .catch(err => {
  ...
  return (
    ...
    <h1 className='text-xl'>Patch</h1>

    <div className='grid gap-4 2xl:grid-cols-3 my-2'>

    
    {patchProduct.map(product => (
    <button onCanPlay={() => handleClick(product)}>
    <div className='border p-2 bg-gray-300 rounded-lg uppercase flex items-center '>
    
        <div>
        {/* <img className='w-36 h-36 rounded-md' src={product.productImg} alt={patchAlt} /> */}
        <img className='w-36 h-36 rounded-md' src={tempImg} alt={patchAlt} />
        </div>
