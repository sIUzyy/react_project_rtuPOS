//IDlace CATEGORY COMPONENT
//THIS PAGE WILL DISPLAY THE ID LACE PRODUCTS
...
import laceAlt from '../Assets/webp-img/idlace-alt.webp'

const IDlace = () => {

  const [idLaceProduct, setIDLaceProduct] = useState([])
  const [pesoSign] = useState('₱')
  const { handleClick, handleSizeSelect} = useContext(ReceiptContext)

  const productRef = collection(db, 'products')
  useEffect(() => {
   const IDLaceProductsQuery = query(productRef, where("productCategory", "==", "ID LACE"));
    
    getDocs(IDLaceProductsQuery)
      .then((snapshot) => {
        let productlists = []
        snapshot.docs.forEach((doc) => {
          productlists.push({...doc.data(), id: doc.id}) 
        })
        setIDLaceProduct(productlists);
      })
      .catch(err => {
  ...
  return (
    ...
    <h1 className='text-xl'>ID Lace</h1>

    <div className='grid gap-4 2xl:grid-cols-3 my-2'>

   
   {idLaceProduct.map(product => (
  <button onClick={() => handleClick(product)}>
  <div className='border p-2 bg-gray-300 rounded-lg uppercase flex items-center '>
    
        <div>
        {/* <img className='w-36 h-36 rounded-md' src={product.productImg} alt={laceAlt} /> */}
        <img className='w-36 h-36 rounded-md' src={tempImg} alt={laceAlt} />
        </div>
