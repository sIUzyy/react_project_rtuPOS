//PANTS CATEGORY COMPONENT
//THIS PAGE WILL DISPLAY THE PANTS PRODUCTS
...
import pantsAlt from '../Assets/webp-img/pants-alt.webp'
...
const Pants = () => {
  ...
  useEffect(() => {
     const pantsProductsQuery = query(productRef, where("productCategory", "==", "PANTS"));
    
    getDocs(pantsProductsQuery)
      .then((snapshot) => {
        let productlists = []
        snapshot.docs.forEach((doc) => {
          productlists.push({...doc.data(), id: doc.id}) 
        })
        setPantsProduct(productlists);
      })
      .catch(err => {
  ...
  return (
   ...
   <button onClick={() => handleClick(product)}>
    <div className='border p-2 bg-gray-300 rounded-lg uppercase flex items-center '>
        ...
        <div>
        {/* <img className='w-36 h-36 rounded-md' src={product.productImg} alt={pantsAlt} /> */}
        <img className='w-36 h-36 rounded-md' src={tempImg} alt={pantsAlt} />
        </div>
